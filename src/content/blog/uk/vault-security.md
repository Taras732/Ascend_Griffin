...
- **[KeePass](https://keepass.info)** — те, чим користуюся сам. Безкоштовно, open-source, локально (база лежить у нас на ноуті у вигляді одного зашифрованого файлу `.kdbx`). Той самий файл можна синхронізувати між пристроями через хмарне сховище — Google Drive, Dropbox, Tailscale — на наш вибір. Майстер-пароль на сам файл, додатково 2FA через keyfile або YubiKey. Існують клієнти під усі платформи: KeePassXC (Win/Mac/Linux), Strongbox (iOS), KeePassDX (Android).
...
- **[VeraCrypt](https://www.veracrypt.fr)** — безкоштовний, перевірений, потужний. Створюємо файл-контейнер на 10 ГБ, монтуємо як окремий диск, працюємо з ним як зі звичайною папкою. Розмонтували — знову один зашифрований файл.
- **[Cryptomator](https://cryptomator.org)** — простіше за VeraCrypt, заточений під шифрування папок у хмарі (Dropbox, Google Drive). Якщо вже зберігаємо файли у хмарі і хочемо зашифрувати — це наш варіант.

Простий патерн:

1. Створюємо зашифрований том з паролем 20+ символів (генеруємо у password manager).
2. Кладемо туди клієнтські матеріали, які захищені NDA.
3. Працюємо з ними тільки коли потрібно — відмонтували після завершення.
4. У vault кладемо **лиш свої нотатки/висновки** з цих матеріалів — без копіювання самих чутливих файлів.
