...
**What it does:** automatic commits every 10 min + push to our repo.
**Savings:** we forget about backup as a problem.
...
**What it does:** lets us create files from a pre-built template with variable substitution (date, project name, etc.).
**Savings:** 5-10 min a day if we keep a daily ritual.
...
**What it does:** gives us a query language (similar to SQL but simpler) we can drop right into a note. The result updates automatically.
**Savings:** instead of walking through 50 files by hand and collecting a list — we get a dashboard that updates itself.
...
**Savings:** 30 seconds every time we need to find a record from a week ago.
...
**What it does:** gives us weekly review, monthly review in one click.
**Savings:** structures our time. Without it, weekly review keeps being "I'll get to it sometime."
...
**What it does:** catches ideas without breaking our current context.
...
**Savings:** minimal daily, but once a quarter it saves us from chaos.
...
**Savings:** if we write a lot in outline format (brainstorming, plans) — significant.
